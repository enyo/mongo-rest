...
(function() {
  ...
  MongoRest = (function() {
    ...
    MongoRest.prototype.getJSONDataName = function(resource, collection) {
      ...
      dataName = collection ? inflection.pluralize(model.modelName) : model.modelName;
      dataName = dataName.charAt(0).toLowerCase() + dataName.slice(1);
      return this.serializeDataObjectKey(this.options.JSONDataNamePrefix + dataName + this.options.JSONDataNameSuffix);
    };

    MongoRest.prototype.serializeDataObject = function(obj, child) {
      var key, serializedObj, value;
      if (child == null) {
        child = false;
      }
      if (!child) {
      ...
      for (key in obj) {
        value = obj[key];
        key = this.serializeDataObjectKey(key);
        value = this.serializeDataValue(value);
        serializedObj[key] = value;
      }
      return serializedObj;
    };

    MongoRest.prototype.serializeDataObjectKey = function(key) {
      if (this.options.camelizeJSONDataKeys) {
        return key;
      }
    ...
    MongoRest.prototype.serializeDataValue = function(value) {
      var val;
      if (value instanceof Array) {
        value = (function() {
          ...
          for (_i = 0, _len = value.length; _i < _len; _i++) {
            val = value[_i];
            _results.push(this.serializeDataValue(val));
          }
          return _results;
        }).call(this);
      } else if (typeof value === "object") {
        value = this.serializeDataObject(value, true);
      }
      return value;
    };

    MongoRest.prototype.deserializeDataObject = function(obj) {
      var deserializedObject, key, value;
      deserializedObject = {};
      for (key in obj) {
        value = obj[key];
        key = this.deserializeDataObjectKey(key);
        value = this.deserializeDataObjectValue(value);
        deserializedObject[key] = value;
      }
      return deserializedObject;
    };

    MongoRest.prototype.deserializeDataObjectKey = function(key) {
      if (this.options.camelizeJSONDataKeys) {
        return key;
      }
      key = key.replace(/_([a-z0-9])/g, function(match, string) {
        return string.toUpperCase();
      });
      return key;
    };

    MongoRest.prototype.deserializeDataObjectValue = function(value) {
      var val;
      if (value instanceof Array) {
        value = (function() {
          var _i, _len, _results;
          ...
          for (_i = 0, _len = value.length; _i < _len; _i++) {
            val = value[_i];
            _results.push(this.deserializeDataObjectValue(val));
          }
          return _results;
        }).call(this);
      } else if (typeof value === "object") {
        value = this.deserializeDataObject(value);
      }
      return value;
    };
    ...
    MongoRest.prototype.getResource = function(pathOrModel) {
      var model, modelName, pathName, resource, _i, _len, _ref;
      ...
      for (_i = 0, _len = _ref.length; _i < _len; _i++) {
        resource = _ref[_i];
        if (resource.pathName === pathName || resource.model === model || resource.model.modelName === modelName) {
          return resource;
        }
      }
      return null;
    ...
    MongoRest.prototype.addInterceptor = function(pathOrModel, eventOrEvents, handler) {
      var event, events, modelName, resource, _i, _len, _results;
      resource = this.getResource(pathOrModel);
      modelName = typeof pathOrModel === "string" ? pathOrModel : pathOrModel.modelName;
      if (!resource) {
        throw new Error("The resource " + modelName + " is not defined!");
      }
    ...
    */
    ...
    MongoRest.prototype.renderCollection = function(docs, req, res, next) {
      ...
      if (resource.enableXhr && req.xhr) {
        ...
        return res.send(this.serializeDataObject(data));
      } else {
    ...
    MongoRest.prototype.collectionPost = function() {
      ...
      return function(req, res, next) {
        ...
        info = {
          values: _this.deserializeDataObject(req.body[req.resource.entityJSONDataName])
        };
