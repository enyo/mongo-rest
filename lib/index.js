...
MongoRest.prototype.renderError = function (err, req, res, next) {
  if (this.options.enableXhr && req.xhr) res.send({ error: err, messages: req.tmpFlashs || null });
  else next(err);
};
...
MongoRest.prototype.renderCollection = function(docs, req, res, next) {
  if (this.options.enableXhr && req.xhr) {
    res.send({ docs: docs, messages: req.tmpFlashs || null });
  }
  else {
 ...
 */
 MongoRest.prototype.renderEntity = function (doc, req, res, next) {
  if (this.options.enableXhr && req.xhr) res.send({ doc: doc, messages: req.tmpFlashs || null });
...
MongoRest.prototype.entityPut = function() { return _.bind(function(req, res, next) {
  ...
  this.invokeInterceptors(req.resource.singularName, 'put', info, req, res, next, function() {
    ...
    req.doc.save(function(err) {
      ...
      self.invokeInterceptors(req.resource.singularName, event, info, req, res, next, function() {
        self.redirect(self.getEntityUrl(req.resource, req.doc), req, res, next);
      });
    });
  });
