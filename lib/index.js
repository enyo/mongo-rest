...
MongoRest.prototype.invokeInterceptors = function(singularResourceName, event, info, req, res, next, onFinish) {
  ...
  _.each(interceptors[singularResourceName][event], function(interceptor) {
...
/**
 * This is a buffer for `req.flash` since xhrs get the messages returned not flashed.
 * 
 * If the request is not xhr, it gets forwarded to `req.flash()` directly.
 * 
 * @param  {String} type 
 * @param  {String} msg 
 * @param  {Req} req 
...
MongoRest.prototype.flash = function(type, msg, req) {
  if (!req.xhr || !this.options.enableXhr) {
    ...
    req.tmpFlashs || (req.tmpFlashs = []);
    req.tmpFlashs.push({ type: type, msg: msg });    
  }
}
...
MongoRest.prototype.renderError = function (err, redirectUrl, req, res, next) {
  if (this.options.enableXhr && req.xhr) {
    ...
    req.tmpFlashs && (obj.messages = req.tmpFlashs);
    res.send(obj);
  }
  else {
    if (redirectUrl) this.redirect(redirectUrl, req, res, next);
    else next(err);
  }
};
...
MongoRest.prototype.collectionPost = function() { return _.bind(function(req, res, next) {
  ...
  this.invokeInterceptors(req.resource.singularName, 'post', info, req, res, next, function() {
    var doc = new req.resource.model(info.values);

    doc.save(function(err) {
      var redirectUrl = self.getCollectionUrl(req.resource);

      info.doc = doc;
      if (err) {
        info.err = err;
        self.flash('error', 'Error: ' + err.message, req)
        self.invokeInterceptors(req.resource.singularName, 'post.error', info, req, res, next, function() {
          self.renderError(err, redirectUrl, req, res, next);
        });
      }
      else {
        self.invokeInterceptors(req.resource.singularName, 'post.success', info, req, res, next, function() {
...
MongoRest.prototype.entity = function() { return _.bind(function(req, res, next) {
  ...
  req.resource.model.findOne({ _id: req.params.id }, function(err, doc) {
    if (err) {
      self.flash('error', err.message, req);
      self.redirect(self.getCollectionUrl(req.resource), req, res, next);
      return;
    }
    req.doc = doc;
    next();
...
MongoRest.prototype.entityPut = function() { return _.bind(function(req, res, next) {
  ...
  this.invokeInterceptors(req.resource.singularName, 'put', info, req, res, next, function() {
    _.each(info.values, function(value, name) {
      req.doc[name] = value;
    });

    req.doc.save(function(err) {
      var redirectUrl = self.getEntityUrl(req.resource, req.doc);

      if (err) {
        info.err = err;
        self.flash('error', 'Unable to save the record: ' + err.message, req)
        self.invokeInterceptors(req.resource.singularName, 'put.error', info, req, res, next, function() {
          self.renderError(err, redirectUrl, req, res, next);
        });
      }
      else {
        self.flash('success', 'Successfully updated the record.', req)
        self.invokeInterceptors(req.resource.singularName, 'put.success', info, req, res, next, function() {
          self.redirect(redirectUrl, req, res, next);
        });
      }
...
MongoRest.prototype.entityDelete = function() { return _.bind(function(req, res, next) {
  ...
  this.invokeInterceptors(req.resource.singularName, 'delete', info, req, res, next, function() {
    req.doc.remove(function(err) {
      var event;
      if (err) {
        event = "delete.error";
        info.err = err;
        self.flash('error', 'Unable to delete the record: ' + err.message, req)
      }
      else {
        event = "delete.success";
        self.flash('success', 'Successfully deleted the record.', req)
      }
      self.invokeInterceptors(req.resource.singularName, event, info, req, res, next, function() {
        self.redirect(self.getCollectionUrl(req.resource), req, res, next);
      });
    });
  });
