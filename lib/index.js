...
(function() {
  ...
  MongoRest = (function() {
    ...
    MongoRest.prototype.addResource = function(singularName, model, options) {
      var key, pluralName, resource, value, _ref, _ref1, _ref2;
      if (options == null) {
        options = {};
      }
      ...
      resource = {
        ...
        pluralName: (_ref = options.pluralName) != null ? _ref : singularName + "s",
        model: model
      };
      if (options.sort != null) {
        resource.sort = options.sort;
      }
      if (resource.pluralName === resource.singularName) {
      ...
      for (key in _ref1) {
        value = _ref1[key];
        if (key !== "urlPath") {
          resource[key] = (_ref2 = options[key]) != null ? _ref2 : value;
        }
      }
      resource.entityViewTemplate = this._substituteNames(resource.entityViewTemplate, resource);
      resource.collectionViewTemplate = this._substituteNames(resource.collectionViewTemplate, resource);
      resource.entityDataName = this._substituteNames(resource.entityDataName, resource);
      resource.collectionDataName = this._substituteNames(resource.collectionDataName, resource);
      return this.resources.push(resource);
    };

    MongoRest.prototype._substituteNames = function(str, resource) {
      return str.replace("{{singularName}}", resource.singularName).replace("{{pluralName}}", resource.pluralName);
    };
    ...
    */
    ...
    MongoRest.prototype.renderCollection = function(docs, req, res, next) {
      ...
      if (resource.enableXhr && req.xhr) {
        ...
        return res.render(resource.collectionViewTemplate, data);
      }
    };
    ...
    MongoRest.prototype.renderEntity = function(doc, req, res, next) {
      ...
      if (resource.enableXhr && req.xhr) {
        ...
        return res.render(resource.entityViewTemplate, data);
      }
    };
