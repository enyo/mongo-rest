    ...
    doc.save(function(err) {
      ...
      if (err) {
        ...
        self.invokeInterceptors(req.resource.singularName, 'post.error', info, req, res, next, function() {
          self.renderError(new Error('Unable to insert the record: ' + err.message), redirectUrl, req, res, next);
        });
      }
      else {
    ...
    req.doc.save(function(err) {
      ...
      if (err) {
        ...
        self.invokeInterceptors(req.resource.singularName, 'put.error', info, req, res, next, function() {
          self.renderError(new Error("Unable to save the record: " + err.message), redirectUrl, req, res, next);
        });
      }
      else {
...
MongoRest.prototype.entityDelete = function() { return _.bind(function(req, res, next) {
  ...
  this.invokeInterceptors(req.resource.singularName, 'delete', info, req, res, next, function() {
    req.doc.remove(function(err) {
      ...
      if (err) {
        info.err = err;
        self.invokeInterceptors(req.resource.singularName, "delete.error", info, req, res, next, function() {
          self.renderError(new Error('Unable to delete the record: ' + err.message), redirectUrl, req, res, next);
        });
      }
      else {
        self.flash('success', 'Successfully deleted the record.', req)
        self.invokeInterceptors(req.resource.singularName, "delete.success", info, req, res, next, function() {
          self.redirect(redirectUrl, req, res, next);
        });
      }
    });
